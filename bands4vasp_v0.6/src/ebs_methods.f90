...
SUBROUTINE PARSE_ION_SELECTION(LINE)
 CHARACTER(LEN=*), INTENT(IN) :: LINE
 CHARACTER(LEN=50), DIMENSION(:), ALLOCATABLE :: TOKENS
 INTEGER, DIMENSION(:), ALLOCATABLE :: VALUES, UNIQUE_VALUES
 INTEGER :: I,J,N,NTOK,POS,FIRST,LAST,IOST,NUNIQUE

 IF(ALLOCATED(PAR%SIONS)) DEALLOCATE(PAR%SIONS)
 PAR%SION=(/ 0,0 /)
 PAR%NSION=0
 NTOK=NITEMS(LINE)
 IF(NTOK<1) RETURN
 ALLOCATE(TOKENS(NTOK))
 READ(LINE,*,IOSTAT=IOST) TOKENS
 IF(IOST/=0) GOTO 900

 N=0
 DO I=1,NTOK
   POS=INDEX(TRIM(TOKENS(I)),'-')
   IF(POS>0)THEN
     IF(POS==1.OR.POS==LEN_TRIM(TOKENS(I)).OR. &
        INDEX(TOKENS(I)(POS+1:),'-')>0) GOTO 900
     READ(TOKENS(I)(:POS-1),*,IOSTAT=IOST) FIRST
     IF(IOST/=0) GOTO 900
     READ(TOKENS(I)(POS+1:),*,IOSTAT=IOST) LAST
     IF(IOST/=0.OR.FIRST<1.OR.LAST<FIRST) GOTO 900
     N=N+LAST-FIRST+1
   ELSE
     READ(TOKENS(I),*,IOSTAT=IOST) FIRST
     IF(IOST/=0.OR.FIRST<0) GOTO 900
     IF(FIRST==0.AND.NTOK/=1) GOTO 900
     IF(FIRST==0) RETURN
     N=N+1
   END IF
 END DO
 ...
 DO I=1,NTOK
   POS=INDEX(TRIM(TOKENS(I)),'-')
   IF(POS>0)THEN
     READ(TOKENS(I)(:POS-1),*) FIRST
     READ(TOKENS(I)(POS+1:),*) LAST
   ELSE
     READ(TOKENS(I),*) FIRST
     LAST=FIRST
   END IF
 ...
 NUNIQUE=1
 DO I=2,N
   IF(VALUES(I)/=VALUES(NUNIQUE))THEN
     NUNIQUE=NUNIQUE+1
     VALUES(NUNIQUE)=VALUES(I)
   END IF
 END DO
 ALLOCATE(UNIQUE_VALUES(NUNIQUE))
 UNIQUE_VALUES=VALUES(:NUNIQUE)
 CALL MOVE_ALLOC(UNIQUE_VALUES,PAR%SIONS)
 PAR%NSION=NUNIQUE
 PAR%SION(1)=PAR%SIONS(1)
 IF(NUNIQUE>1) PAR%SION(2)=PAR%SIONS(NUNIQUE)
 RETURN

900 CONTINUE
 WRITE(*,'(A,A)') 'WARNING: Invalid ion SELECTION: ',TRIM(LINE)
 WRITE(*,'(A)') '         Use 0, positive ion numbers, and ascending ranges (for example: 1 3 7-10).'
 WRITE(*,'(A)') '         Set SELECTION to 0 (all ions).'
 IF(ALLOCATED(PAR%SIONS)) DEALLOCATE(PAR%SIONS)
 PAR%SION=(/ 0,0 /)
 PAR%NSION=0
END SUBROUTINE PARSE_ION_SELECTION


SUBROUTINE VALIDATE_ION_SELECTION()
 INTEGER, DIMENSION(:), ALLOCATABLE :: VALID
 INTEGER :: NVALID,MAXION

 IF(PAR%NSION<1) RETURN
 MAXION=MINVAL(NFILE(:)%IONS)
 NVALID=COUNT(PAR%SIONS<=MAXION)
 IF(NVALID<PAR%NSION)THEN
   WRITE(*,'(A,I0,A)') 'WARNING: Ignoring selected ions above ',MAXION, &
                       '; they are not present in every input file.'
 END IF
 IF(NVALID==0)THEN
   WRITE(*,'(A)') 'WARNING: No selected ion is present in every input file.'
   WRITE(*,'(A)') '         Set SELECTION to 0 (all ions).'
   DEALLOCATE(PAR%SIONS)
   PAR%SION=(/ 0,0 /)
   PAR%NSION=0
   RETURN
 END IF
 ALLOCATE(VALID(NVALID))
 VALID=PACK(PAR%SIONS,PAR%SIONS<=MAXION)
 CALL MOVE_ALLOC(VALID,PAR%SIONS)
 PAR%NSION=NVALID
 PAR%SION(1)=PAR%SIONS(1)
 PAR%SION(2)=0
 IF(NVALID>1) PAR%SION(2)=PAR%SIONS(NVALID)
END SUBROUTINE VALIDATE_ION_SELECTION
...
SUBROUTINE READ_INPAR(INPAR)
 ...
 END IF
  ...
  END IF
  READ(11,'(A)') CHARA
  IF((CHARA(1:1)=='#').OR.(LEN_TRIM(CHARA))<6)THEN
    PAR%IREFLECT=1
    READ(11,'(A)') CHARA
    IF((CHARA(1:1)/='#').AND.(LEN_TRIM(CHARA)>=6))THEN
 ...
 ELSE
  ...
  READ(11,'(A)') SELECTION_LINE
  CALL PARSE_ION_SELECTION(SELECTION_LINE)
...
SUBROUTINE READBODY_PROCAR_PRIM(FILEDIR,TMPDIR,TEBS,KGRID,LATTICE,LUNFOLD,LFINISH)
 ...
 END IF
 CALL VALIDATE_ION_SELECTION()
 IF(PAR%NSION>0)THEN
   WRITE(*,'(A,I0,A)',ADVANCE='NO') 'Selected IONs for projection (',PAR%NSION,') ::'
   DO I=1,PAR%NSION
     WRITE(*,'(1X,I0)',ADVANCE='NO') PAR%SIONS(I)
   END DO
   WRITE(*,*) ''
 ELSE
   WRITE(*,'(A)') 'Selected all IONs for projection.'
 END IF
 WRITE(*,*) ''
     ...
     spin: DO SPN=1,2
       ...
       KPOINTS: DO B=1,NFILE(P)%KPTS
         ...
         bnd: DO I=1,NFILE(P)%BND
          ...
          IF ((ENERGY>=PAR%FBORDER(1)).AND.(ENERGY<=PAR%FBORDER(2).AND.(BLOCH>PAR%TRASH)))THEN
            ...
            DO J=1,NFILE(P)%IONS
              ...
              DO ND=1,NFILE(P)%ORBIT
                ...
                ELSE
                  ...
                  IF(ANY(PAR%SIONS==NION)) ORBITS(ND)=ORBITS(ND)+SORBIT
                END IF
              END DO
              READ(UN,'(F6.3)') STOT
              IF(ANY(PAR%SIONS==NION)) TOT=TOT+STOT
              ORBITS(NFILE(P)%ORBIT+1)=TOT

            END DO

            READ(UN,*) ! Ignore the total over all ions
            READ(UN,*)
          ELSE
    ...
    OPEN(UNIT=UN, FILE=TRIM(DATANAME), STATUS='OLD',  ACTION='READ')
     ...
     spins: DO WHILE (I<=NFILE(P)%SPIN)
       ...
       kpoint: DO WHILE (K<=NFILE(P)%KPTS)    
         IF(CDUM(1:7)=='k-point')THEN
           READ(CDUM,'(33X,I6,7X,3F14.7)') K,NVKPT

           !check if k-point need to skip
           IF((K>=PAR%KSKIP(1)).AND.(K<=PAR%KSKIP(2)))THEN
             K=K+1
             IOCHECK=0
             READ(UN,'(A)',IOSTAT=IOCHECK) CDUM
             DO WHILE ( (CDUM(1:7)/='k-point').AND.(IOCHECK==0))
               READ(UN,'(A)',IOSTAT=IOCHECK) CDUM
             END DO
             CYCLE
           END IF
           ...
           IF(LMINV)THEN
             CALL CHANGE_BASIS(M_INV,NVKPT,VKPT)
             KEFF=KEFF+1
             IF(LKFIRST)THEN
               KSCF0=VKPT
               LKFIRST=.FALSE.
             END IF
             KMAP(K)=KEFF
             KDIST(K)=KDISTANCE(VKPT,KSCF0,LATTICE%B)
           END IF
           ...
           bands: DO WHILE (BND<=NFILE(P)%BND)
             ...
             IF(CDUM(1:5)=='band:')THEN
               ...
               IF((ENERGY<0.0_DP).AND.(ENERGY>N%ENMAX(I)).AND.(KAPPA_SUM>PAR%TRASH))THEN
                 N%ENMAX(I)=ENERGY
                 N%LENMAX(I)=.TRUE.
               ELSE IF ((ENERGY>=0.0_DP).AND.(ENERGY<N%EPMIN(I)).AND.(KAPPA_SUM>PAR%TRASH))THEN
                 N%EPMIN(I)=ENERGY
                 N%LEPMIN(I)=.TRUE.
               END IF          
...
SUBROUTINE FINDFITTPOINTS(FP, LFIT, SPN)
 ...
 INTEGER :: I,J,K,P,NK,NJ,BNJ,NA,KK,NFP,NCROS,NN,SPN,NT,KSAVE
 INTEGER, DIMENSION(2*PAR%POINTS,2) :: HITNUM
 REAL(DP) :: WEIGHT,DDIST,DGRAD,DALPHA,UPE,DOWNE
 REAL(DP), DIMENSION(2*PAR%POINTS-1) :: GRAD
 ...
 DO P=1,N%PATH
   ...
   kpoints: DO
     ...
     samek: DO J=1,N%SAMEK
       IF(LPOS(K,J).AND.((EBS(K,J,SPN)%E>=PAR%WDIF(1)).AND.(EBS(K,J,SPN)%E<=PAR%WDIF(2))))THEN
         LEXIST1=.TRUE.
         DO WHILE ( LEXIST1 )
           CALL NEIGHBOUR(EBS(K,J,SPN)%E,K,EBS(K,J,SPN)%BND,LEXIST1,EPOINT,NJ,SPN,.TRUE.)
           ...
           IF ( LEXIST1 ) THEN
             ...
             & .AND. (ORBIT_DIF(EBS(K,J,SPN)%OC(:),EPOINT%OC(:)))) THEN
              ...
              CALL NEIGHBOUR(EPOINT%E,-K-1,EPOINT%BND,LEXIST2,BEPOINT,BNJ,SPN,.FALSE.)
              IF(LEXIST2)THEN
               ...
               DO NA = PAR%POINTS+2,NFP,1
                 LEXIST2=.TRUE.
                 DO WHILE (LEXIST2)
             CALL NEIGHBOUR(TFP(NA-1)%E+GRAD(NA-2),KK,TFP(NA-1)%BND,LEXIST2,EPOINT,NJ,SPN,.FALSE.)
!
                   IF (LEXIST2) THEN
                     GRAD(NA-1)=(EPOINT%E-TFP(NA-1)%E)
                     UPE=TFP(NA-1)%E+GRAD(NA-2)+PAR%DGRAD*PAR%EGAP
                     UPE=MIN(UPE,PAR%WDIF(2))
                     DOWNE=TFP(NA-1)%E+GRAD(NA-2)-PAR%DGRAD*PAR%EGAP
               ...
               DO NA = PAR%POINTS-1,1,-1
                 LEXIST2=.TRUE.
                 DO WHILE (LEXIST2)
          CALL NEIGHBOUR(TFP(NA+1)%E-GRAD(NA+1),-KK,TFP(NA+1)%BND,LEXIST2,EPOINT,NJ,SPN,.FALSE.)
                   IF (LEXIST2) THEN
                      GRAD(NA)=TFP(NA+1)%E-EPOINT%E
                       UPE=TFP(NA+1)%E-GRAD(NA+1)+PAR%DGRAD*PAR%EGAP
                     UPE=MIN(UPE,PAR%WDIF(2))
                       DOWNE=TFP(NA+1)%E-GRAD(NA+1)-PAR%DGRAD*PAR%EGAP
                     DOWNE=MAX(DOWNE,PAR%WDIF(1))
...
SUBROUTINE NEIGHBOUR(ENERGY,K,BND,LEXIST,NEBS,NJ,SPN,LCROS)
 TYPE( BAND ) :: NEBS
 REAL(KIND=DP) :: ENERGY
 INTEGER :: K, NK, J, NJ, NKO,BND,SPN
 LOGICAL :: LEXIST, FIRST, LCROS
  FIRST=.TRUE.
  LEXIST=.FALSE.
  NK=ABS(K+1)
  ...
  DO J=1,N%SAMEK
    IF(LPOS(NK,J))THEN
      ...
      IF(LCROS.AND.(ENERGY*EBS(NK,J,SPN)%E>0.0_DP))CYCLE
      IF (FIRST) THEN
        NEBS=EBS(NK,J,SPN)
        NJ=J
        LEXIST=.TRUE.
...
SUBROUTINE POLYNOM_INTERPOL_ROOTS( FP, ROOTS, PLOTFILE, KGRID, SPN)
 ...
 REAL(DP), DIMENSION(:,:), ALLOCATABLE :: AN,XY,X12,XTREME
 ...
 ALLOCATE(AN(DFIT,NP),X12(NP*DFIT,2),MAP(NP*DFIT),OCHAR(N%ORB),CON(N%ORB),XTREME(NP*DFIT,2))
 OPEN(UNIT=16, FILE=TRIM(PLOTFILE), STATUS='REPLACE', ACTION='WRITE')
  DO KA = 1,DFIT
    ...
    DO
      F=POLYNOM(A,X)
      IF((F<=PAR%WDIF(1)).OR.( F>=PAR%WDIF(2)))THEN
        X=X+DX
  ...
  DO I=1,NFIT
    ...
    ROOTS(I)%K=REGULA_FALSI(A,X1,X2,PAR%EPS,XTREME(I,1),XTREME(I,2))
    DO J=1,NP
...
SUBROUTINE WRITE_FERMIFILE(FERMIFILE,ROOTS,ROOTSXYZ,KGRID,LATTICE,FILETYPE)
  ...
  IF(FILETYPE/=3)THEN
  IF(PAR%NSION>0)THEN
    WRITE(851,'(A)',ADVANCE='NO') ' and IONS'
    DO I=1,PAR%NSION
      WRITE(851,'(1X,I0)',ADVANCE='NO') PAR%SIONS(I)
    END DO
    WRITE(851,*) ''
  ELSE
