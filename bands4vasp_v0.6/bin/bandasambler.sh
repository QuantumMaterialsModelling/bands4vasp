...
spinpalette="( -1 'blue', 0 'white', 1 'red')"
...
# For spin-polarized calculations, also overlay both spin channels. The
# individual .spin1 and .spin2 plots above remain unchanged.
if [ "$totspin" -eq 2 ] && [ "$spin" = ".spin2" ]; then
  plotdata_up="${plotdata/.spin2.dat/.spin1.dat}"
  if [ -f "$plotdata_up" ] && [ -f "$plotdata" ]; then
    if [ "$lfit" -eq 4 ] || [ "$lfit" -eq 5 ]; then
      spinboth="${bandstructure%.spin2}.spins"
    else
      spinboth="${ebsbloch%.spin2}.spins"
    fi
    echo "$spinboth.$prefix" >> "$created"
    cat >> "$output" <<catOUT
unset multiplot
reset
$terminal
set output '$spinboth.$prefix'
...
catOUT
    if [ "$lxrange" = true ]; then echo "set xrange [$x0:$x1]" >> "$output"; fi
    echo "plot '$plotdata_up' u 1:2 w p pt 7 ps $statfp lc rgb 'red' title 'spin up', \\" >> "$output"
    echo "     '$plotdata' u 1:2 w p pt 7 ps $statfp lc rgb 'blue' title 'spin down', \\" >> "$output"
    echo "     '$outgrid' w l ls 0 notitle, 0 ls 0 notitle" >> "$output"
  fi
fi
...
# Combined spin-resolved projection plots. Spin-up weights are positive and
# spin-down weights are negated, giving a blue-white-red scale centered at zero.
if [ "$totspin" -eq 2 ] && [ "$spin" = ".spin2" ]; then
  plotdata_up="${plotdata/.spin2.dat/.spin1.dat}"
  orbitalstats_up="${orbitalstats/.spin2.dat/.spin1.dat}"
  if [ -f "$plotdata_up" ] && [ -f "$plotdata" ]; then
    maxorb_up=$(awk '/#maxorb / {print $2}' "$orbitalstats_up" 2>/dev/null)
    maxorb_both=$(awk -v a="${maxorb_up:-0}" -v b="${maxorb:-0}" 'BEGIN {if (a>b) print a; else print b}')
    if awk -v m="$maxorb_both" 'BEGIN {exit !(m>0)}'; then
      combined_ebsorbit="${ebsorbit%.spin2}.spins"
      combined_layout="$orblayout"
      if [ "$norbis" -gt 1 ]; then
        combined_layout="layout $(grep '#LAYOUT=' "$orbitalstats" | cut -d'=' -f2) margins 0.05,0.90,0.09,0.94 spacing 0.04,0.08"
      fi
      echo "$combined_ebsorbit.$prefix" >> "$created"
      cat >> "$output" <<catOUT
unset multiplot
reset
$terminal
set output '$combined_ebsorbit.$prefix'
unset key
set multiplot $combined_layout
set object 1 rect from graph 0,graph 0 to graph 1,graph 1 back
set object 1 fc rgb '$background' fs solid 1.0 behind
set yrange [$pwind1:$pwind2]
set cbrange [-$maxorb_both:$maxorb_both]
set palette defined $spinpalette
set cbtics font "$vfont"
set xtics $xtics font "$vfont" offset 0,0.5
set ytics font "$vfont" offset 0.2,0
$xlabel
...
catOUT
      if [ "$norbis" -gt 1 ]; then
        cat >> "$output" <<catOUT
set colorbox user origin screen 0.925,0.16 size screen 0.015,0.70
set cblabel '{/:Bold signed projection}' font "$vfont" offset 1.0,0
catOUT
      else
        echo "set cblabel '{/:Bold signed projection (spin down < 0 < spin up)}' font \"$titlefont\"" >> "$output"
      fi
      if [ "$lxrange" = true ]; then echo "set xrange [$x0:$x1]" >> "$output"; fi
      combined_or=0
      while [ "$combined_or" -lt "$norbis" ]; do
        if [ "$selectorb" -gt 0 ] && [ "$nallorbitals" -ge "$selectorb" ]; then
          combined_or=$((selectorb - 1))
          combined_orow="$combined_or"
          combined_title=$(grep -E "#[[:space:]]*$combined_or""ALLORBITS=" "$plotdata" | cut -d'=' -f2)
        else
          combined_title=$(grep -E "#[[:space:]]*$combined_or""ORBITAL=" "$orbitalstats" | cut -d'=' -f2)
          combined_orow=$(grep -nE "#[ ,1-9][0-9]ALLORBITS=" "$plotdata" | grep -E "${combined_title}$" | cut -d':' -f1)
          combined_orow=$((combined_orow - 1))
        fi
        combined_col=$((4 + combined_orow))
        echo "set label 1 at graph 0.45,1.09 '{/:Bold $combined_title}' font '$pfont'" >> "$output"
        if [ "$lfit" -eq 4 ] || [ "$lfit" -eq 5 ]; then
          echo "plot '<sed \"/^#/d\" $plotdata_up | sort -n -k$combined_col' u 1:2:$combined_col w p pt 7 ps $statfp palette, \\" >> "$output"
          echo "     '<sed \"/^#/d\" $plotdata | sort -n -k$combined_col' u 1:2:(-\$$combined_col) w p pt 6 ps $statfp palette, \\" >> "$output"
        else
          echo "plot '<sed \"/^#/d\" $plotdata_up | sort -n -k$combined_col' u 1:2:((\$3*$orbpsfac)*$psfac):$combined_col w p pt 7 ps var palette, \\" >> "$output"
          echo "     '<sed \"/^#/d\" $plotdata | sort -n -k$combined_col' u 1:2:((\$3*$orbpsfac)*$psfac):(-\$$combined_col) w p pt 6 ps var palette, \\" >> "$output"
        fi
        echo "     '$outgrid' w l ls 0 notitle, 0 ls 0 notitle" >> "$output"
        combined_or=$((combined_or + 1))
      done
      echo "unset multiplot" >> "$output"

      # PLOTORB=0 creates the ALL figure. Also emit the total selected-ion
      # projection as a separate, easier-to-read combined-spin plot.
      if [ "$selectorb" -eq 0 ]; then
        tot_header=$(grep -nE '#[[:space:]]*[0-9]+ALLORBITS=tot$' "$plotdata" | head -n1 | cut -d':' -f1)
        if [ -n "$tot_header" ]; then
          tot_col=$((tot_header + 3))
          combined_tot="${ebsorb}_tot.spins"
          echo "$combined_tot.$prefix" >> "$created"
          cat >> "$output" <<catOUT
unset multiplot
reset
$terminal
set output '$combined_tot.$prefix'
unset key
set object 1 rect from graph 0,graph 0 to graph 1,graph 1 back
set object 1 fc rgb '$background' fs solid 1.0 behind
set yrange [$pwind1:$pwind2]
set cbrange [-$maxorb_both:$maxorb_both]
set palette defined $spinpalette
set cblabel '{/:Bold signed total projection (spin down < 0 < spin up)}' font "$titlefont"
set cbtics font "$vfont"
set xtics $xtics font "$vfont" offset 0,0.5
set ytics font "$vfont" offset 0.2,0
set label 1 at graph 0.45,1.06 '{/:Bold tot}' font '$pfont'
$xlabel
$ylabel
catOUT
          if [ "$lxrange" = true ]; then echo "set xrange [$x0:$x1]" >> "$output"; fi
          if [ "$lfit" -eq 4 ] || [ "$lfit" -eq 5 ]; then
            echo "plot '<sed \"/^#/d\" $plotdata_up | sort -n -k$tot_col' u 1:2:$tot_col w p pt 7 ps $statfp palette, \\" >> "$output"
            echo "     '<sed \"/^#/d\" $plotdata | sort -n -k$tot_col' u 1:2:(-\$$tot_col) w p pt 6 ps $statfp palette, \\" >> "$output"
          else
            echo "plot '<sed \"/^#/d\" $plotdata_up | sort -n -k$tot_col' u 1:2:((\$3*$orbpsfac)*$psfac):$tot_col w p pt 7 ps var palette, \\" >> "$output"
            echo "     '<sed \"/^#/d\" $plotdata | sort -n -k$tot_col' u 1:2:((\$3*$orbpsfac)*$psfac):(-\$$tot_col) w p pt 6 ps var palette, \\" >> "$output"
          fi
          echo "     '$outgrid' w l ls 0 notitle, 0 ls 0 notitle" >> "$output"
        fi
      fi
