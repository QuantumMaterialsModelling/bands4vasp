...
#========================== esp ============================
  ...
  cbspeclabel="set cblabel '{/:Bold A}({/:Bold k},E_{F})' "
  if [ -z "$plotsize" ];then
     cbfspeclabel="$cbspeclabel font '$pfont' offset graph -0.13,graph 0.01"
...
if [ $fnorbis -gt 1 ];then
  ...
  fopfsize=`echo "scale=1; $topfsize-(7*$fops)/1"|bc`
...
if [ $lfit -eq 4 -o $lfit -eq 5 ]; then
  echo "set out '$bandstructure.processed.$prefix'" >> $output
  echo "$bandstructure.processed.$prefix" >> $created
else
  echo "set out '$ebsbloch.processed.$prefix'" >> $output
  echo "$ebsbloch.processed.$prefix" >> $created
  if $userbpalette;then
    echo "set palette defined (0 '$bcolour1', 1 '$bcolour2')" >> $output
  else
    echo "set palette defined $blochpalette" >> $output
...
unset xtics
unset ytics
set label 1 at graph 0.5,1.1 '{/:Bold $orbitaltitel}' font '$fopfont'
catend
if $florb;then
  if [ `sed '/#/d' $fsurdat|grep -cE '[0-9]'` -gt 0 ];then
