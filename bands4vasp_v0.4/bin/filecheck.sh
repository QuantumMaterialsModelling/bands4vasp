...
#filename is a number
  ...
  if [ "$sfile" == "OUTCAR" ];then
    if [[ `echo "$filename"|grep -E '^-?[0-9]+\.*[0-9]*$'` ]];then
      echo "$filename" > $ftf
      astat=2
    fi
  fi
...
#********************************************************
  ...
  if [ $nsep -gt 0 ];then filename=$(rclean `echo "$procar"|cut -d'/' -f1-$nsep`);fi
  if [ -z $filename -o $nsep -eq 0 ];then filename='.';fi
  if [ $promulti -eq 0 ];then
...
elif [[ `echo $fpastat|grep -E '^-?\.[0-9]+$|^-?[0-9]+\.?[0-9]*$'` ]];then
