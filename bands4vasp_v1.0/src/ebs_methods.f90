...
LOGICAL FUNCTION WRITE_PLOTDATA(PLOTFILE,ACOLBAND,TEBS,FILETYPE,SPINFILE)
 ...
 CHARACTER(LEN=60) :: PFILE,SPFILE,BPFILE,EMPTYORBS
 CHARACTER(LEN=6) :: CSPN
 REAL(DP) :: MAXORB
 INTEGER :: B, K, I,SPN,NBANDS, FILETYPE,OR,OBEGIN,OEND,ACOLEN,EOPOS,EOLEN
 ...
 IF(FILETYPE/=3)THEN
   IF(PAR%FORB==0)THEN
     OBEGIN=1
     OEND=N%ORB
     IF((PAR%SION(1)<1).AND.(PAR%SION(2)<1)) N%ORB=N%ORB-1
   END IF
   MAXORB=0.0_DP
   DO I=1,SIZE(TEBS,1)
...
!     IF(ANY(NOORB))THEN
   ...
   IF(I==0)THEN
     ...
     WRITE(*,*) ''
   ELSE
     WRITE(65,'(A,F4.2)') '#maxorb ',maxorb
     IF((ANY(NOORB(1:N%ORB))).AND.(PAR%FORB==0))THEN
       WRITE(65,'(A)') '#emptyorbs '//EMPTYORBS(1:EOLEN)
...
!    IF(ANY(NOORB))THEN
  ...
  IF(I==0)THEN
    ...
    IF(PAR%SPN==2)THEN
      WRITE(*,'(A,I1,A)') 'Manipulated data (spin=',SPN,'):: No orbitalcharcter greater than 0.01 found'
    ELSE
      WRITE(*,'(A)') 'Manipulated data:: No orbitalcharcter greater than 0.01 found'
    END IF
    WRITE(*,*) ''
  END IF
END IF
...
SUBROUTINE FERMISURFACE(TEBS,OROOTS,ROOTSXYZ,KGRID,LATTICE,BANDSPEC,FERMIFILE,SPECFILE,FILETYPE,SPN)
 ...
 CHARACTER(LEN=60) :: FFILE,SFILE,BFILE,SPM3DFILE
 CHARACTER(LEN=11) :: OCHAR
 CHARACTER(LEN=6) :: CNUM
 REAL(DP),PARAMETER :: THIN=0.000001_DP,PI=2.0_DP*ATAN(1.0)
 ...
 REAL(DP),DIMENSION(3) :: ORIGIN,VEC0,NVEC,RNVEC,VEC1,VEC2,AREAL,AREAL1,BREAL,BREAL1
 REAL(DP),DIMENSION(2) :: OVEC,LVEC,SVEC,FXY,XBORDER,YBORDER
 REAL(DP) :: ANGLE,OANGLE,DOTPRO,XDIST,XMAX,YDIST,YMAX,XRANGE,YRANGE,D,DTEMP,AS,AO,ZMIN,ZMAX
...
!************************* Now the basisvectors are known and stored in PX and RNVEC ********************
...
DO P=1,N%PATH
  VEC1=DMATMUL(PX,CGRID(P)%A-ORIGIN)
  VEC2=DMATMUL(PX,CGRID(P)%B-ORIGIN)
  IF(P==1)THEN
...
!**************** Check the Symmetry option *********************
 IF(PAR%IREFLECT>1)THEN
  ...
  IF (ABS(DDOT_PRODUCT(REALSYMP(1,:)-ORIGIN,RNVEC))>THIN)THEN
     ...
     IF (ABS(DDOT_PRODUCT(REALSYMP(2,:)-ORIGIN,RNVEC))>THIN)THEN
       CALL CLEAN_DISPLAY_LINE()
   ...
   IF(PAR%IREFLECT==2)THEN
     IF(ABS(KDISTANCE(PAR%SYMPOINTS(2,:),PAR%SYMPOINTS(1,:),LATTICE%B))<THIN)THEN
       CALL CLEAN_DISPLAY_LINE()
...
!************ Get the x-y-coordinates on the plane *********************
 IF(N%FIT(SPN)>0)THEN
   DO I=1,N%FIT(SPN)
     VEC1=DMATMUL(PX,ROOTSXYZ(I,:)-ORIGIN)
...
VEC1=DMATMUL(PX,REALSYMP(1,:)-ORIGIN)
OVEC(1:2)=VEC1(1:2)
  ...
  SPIN: DO SPN=1,PAR%SPN
    ...
    N%SPEC(SPN)=0
     ...
     OPEN(UNIT=UNI,FILE=TRIM(SPECFILE)//'.dat',RECL=300,status="unknown")
      ...
      IF((PAR%LSURFACE).AND.(N%SPEC(SPN)==0))THEN
        IF(PAR%SPN==2)THEN
