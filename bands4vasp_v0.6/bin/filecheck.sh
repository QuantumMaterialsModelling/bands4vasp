...
getdir() {
  ...
  if [ "$lmultifile" -gt 0 ]; then
    stat=1
    if [ -n "$last" ]; then
      # A pattern such as kpath00% normally denotes matching directories.
      # Try the requested file inside those directories before interpreting
      # the pattern itself as a direct file pattern. This also makes the
